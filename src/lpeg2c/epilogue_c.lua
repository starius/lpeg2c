...
size_t lpeg_initposition(lua_State *L, size_t len) {
    lua_Integer ii = luaL_optinteger(L, 2, 1);
    /* positive index? */
    if (ii > 0) {
        /* inside the string? */
        if ((size_t)ii <= len) {
...
// Arguments:
// 1. string (subject)
// 2. init position (optional)
// Results:
// TODO
int lua_lpeg2c_match(lua_State* L) {
    size_t l;
    ...
    mstate.captop = 0;
    mstate.ptop = ptop;
    const char* r = lpeg2c_match(&mstate);
    if (r == NULL) {
        lua_pushnil(L);
        return 1;
    }
...
int luaopen_pattern(lua_State* L) {
    lua_pushcfunction(L, lua_lpeg2c_match);
    return 1;
}
